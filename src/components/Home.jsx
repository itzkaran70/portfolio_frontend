...
//     axios.get(`${API_BASE_URL}profiles/`) // Change the id as needed
//       .then(response => {
...
function Home() {
  ...
  const [typedStrings, setTypedStrings] = useState([]);
  const [profile, setProfile] = useState(null);
  const [profileId, setProfileId] = useState(null);  // State to hold the profile ID
  ...
  useEffect(() => {
    ...
    // Fetch the profile data from the backend
    if (profileId) {
      axios.get(`${API_BASE_URL}profiles/${profileId}`) // Use the profileId in the request
        .then(response => {
          console.log('Profile Data:', response.data);
          setProfile(response.data);
        })
      ...
      console.log('Profile ID is not set.');
    }
  }, [profileId]);  // Include profileId in the dependency array
